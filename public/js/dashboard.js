$(document).ready(function () {
  ...
  function loadTasks() {
    console.log('Chargement des t├óches...');
    $.ajax({
      url: '/taches',
      method: 'GET',
      success: data => {
        console.log('Donn├®es re├ºues:', data);
        const tbody = $('#tableau-taches tbody');
        tbody.empty();
  
        data.forEach(task => {
          // V├®rifier si la t├óche a des fichiers
      ...
      error: () => alert('Erreur de chargement des t├óches')
    });
  }

  // Ajouter une nouvelle t├óche avec fichiers
  $('#valider-tache').on('click', () => {
    ...
    $.ajax({
      ...
      success: (task) => {
        ...
        afficherNotification("T├óche ajout├®e avec succ├¿s.", "success");

        // R├®initialiser les champs
        $('#tache-nom').val('');
        $('#tache-priorite').val('moyenne');
        $('#tache-ech').val('');
        $('#tache-fichiers').val(''); // ÔåÉ reset fichiers
        $('#formulaire-ajout').removeClass('visible');
      },
      error: () => {
        afficherNotification("Erreur lors de l'ajout de la t├óche.", "error");
      }
    });
  });
  ...
  loadTasks();
    ...
    // R├®affichage du formulaire de cr├®ation
    ...
    // Bouton d├®connexion (redirige vers logout Laravel)
    $(document).ready(function () {
      $('.btn-logout').on('click', function (e) {
      e.preventDefault(); // Emp├¬che le clic de rediriger ou recharger la page
      $('.logout-form').submit(); // Soumet le form cach├® via jQuery
      });
    });

    // ├ëv├®nement clic sur bouton modifier
    $(document).on('click', '.btn-modify', function () {
      console.log('Bouton de modification cliqu├®');
      const taskId = $(this).closest('tr').data('id');
      $('#modify-task-id').val(taskId);
      $('#modifyModal').css('display', 'flex'); // Afficher le modal personnalis├®
    });
    ...
    $(document).on('click', '.btn-delete', function () {
      taskIdToDelete = $(this).data('id'); // R├®cup├¿re l'ID de la t├óche depuis le bouton
    ...
    $('#btnConfirm').on('click', function () {
      ...
      $.ajax({
        ...
        success: function () {
          ...
          taskIdToDelete = null;
          loadTasks(); // Recharge les t├óches
        },
        error: function () {
          alert('Erreur lors de la suppression de la t├óche.');
        }
      });
    });
    ...
    function chargerTachesTriees(critere, ordre) {
      $.ajax({
        ...
        error: () => alert('Erreur lors du tri des t├óches.')
      });
    }
   
    // Fonction pour charger les t├óches tri├®es
    // Lorsqu'on change la priorit├®
    ...
    $('#search-projects').on('input', function () {
      ...
      fetch(`/taches?search=${encodeURIComponent(search)}`)
        ...
        .then(data => {
          ...
          data.forEach(task => {
            // Construction des fichiers joints (si pr├®sents)
            const fichiersHTML = (task.fichiers && task.fichiers.length > 0)
            ...
            // Construction de la ligne de t├óche
            const row = `
              <tr data-id="${task.id}">
                <td>${task.nom}</td>
                <td class="statut" data-stat="${task.statut}">
